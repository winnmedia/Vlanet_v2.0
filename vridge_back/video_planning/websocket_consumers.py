...
class VideoPlanningCollaborationConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        try:
            ...
            logger.info(f" {self.user.username}  {self.planning_id}   ")
    ...
    async def receive(self, text_data):
        ...
        try:
            ...
            if message_type == 'planning_update':
                ...
                logger.warning(f"    : {message_type}")
                
        except json.JSONDecodeError:
            await self.send_error(" JSON .")
        except Exception as e:
            logger.error(f"  : {str(e)}")
            await self.send_error(f"    : {str(e)}")
    ...
    async def handle_real_time_comment(self, data):
        ...
        try:
            ...
            target_section = data.get('target_section')  # story, scene, shot 
...
class VideoPlanningNotificationConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        try:
            ...
            logger.info(f" {self.user.username}  WebSocket ")
