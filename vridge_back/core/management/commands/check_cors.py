    ...
    def handle(self, *args, **options):
        ...
        for i, middleware in enumerate(settings.MIDDLEWARE):
            if 'cors' in middleware.lower() or 'options' in middleware.lower():
                status = '' if 'solution' in middleware else ''
        ...
        if 'config.cors_solution.OptionsHandlerMiddleware' in settings.MIDDLEWARE:
            ...
            if index == 0:
                ...
                self.stdout.write(self.style.ERROR(f'    OptionsHandlerMiddleware at position {index} - Should be first!'))
        else:
        ...
        if 'corsheaders.middleware.CorsMiddleware' in settings.MIDDLEWARE:
            self.stdout.write(self.style.WARNING('    django-cors-headers middleware still active - Consider removing'))
